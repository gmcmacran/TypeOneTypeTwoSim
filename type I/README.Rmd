...
# Type I Summary
Asymptotic type I error rates for LRTesteR's likelihood ratio tests are estimated via simulation. Calculations are based on 2,000 iterations and a sample size of 2,000. Where possible, exact tests are included for comparison.
...
To check the above, two graphs are shown per test. For the first point, type I error rate is calculated for many areas in the parameter space.For the second point, results are aggregate across alternative hypotheses.
...
typeI <- readRDS("results/empirical_type_one.rds")
