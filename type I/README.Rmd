...
gaussian <- gaussian %>% 
  filter(test %in% c("gaussian_mean_one_sample", "t.test")) %>%
...
link_exact_to_likelihood <- function(test) {
  test2 <- case_when(
    str_detect(test, "t.test") ~ "gaussian_mean_one_sample",
...
temp %>%
  ...
  geom_text() +
  scale_x_continuous(breaks = seq(1, 9, 4)) + 
  scale_y_continuous(breaks = seq(1, 9, 4)) + 
  scale_fill_gradient(low = "white", high = "red", limits = c(0, .10)) +
...
## Negative Binomial
As long as the target number of success is large or p is not near one, the type I error rate is .05. When the target number of successes is small and p is near one, the likelihood test has a slight increase in type I error rate. How near is too near depends on the target number of successes. Visually this is the bottom right corner of the right graph. The exact test shows poor performance in the bottom right corner as well but is always conservative.
