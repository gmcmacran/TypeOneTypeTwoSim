...
Most tests have a type I error rate of .05. Exploring each test one by one, most tests have a consistent type I rate across the explored parameter space and alternative hypotheses. There are 1 minor exception and 2 major exceptions:
...
empLikMu <- readRDS("results/empirical_mu_type_one.rds")
empLikMu <- empLikMu %>%
  select(test, alt, param = mu, pvalue)

empLikVariance <- readRDS("results/empirical_variance_type_one.rds")
empLikVariance <- empLikVariance %>%
  select(test, alt, param = variance, pvalue)

empLikSkew <- readRDS("results/empirical_skewness_type_one.rds")
empLikSkew <- empLikSkew %>%
  select(test, alt, param = skew, pvalue)

empLikKurtosis <- readRDS("results/empirical_kurtosis_type_one.rds")
empLikKurtosis <- empLikKurtosis %>%
  select(test, alt, param = kurtosis, pvalue)
...
ggplot(temp, aes(x = TypeI, y = test2, color = factor(type))) +
  ...
  scale_x_continuous(breaks = seq(-.01, 1.01, .02), limits = c(0, .15)) +
...
### Log Normal
```{r logNormalTypeI, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeI <- readRDS("results/log_normal_type_one.rds")
...
```{r empVarianceTypeI, echo=FALSE}
typeI <- readRDS("results/empirical_variance_type_one.rds")

temp <- typeI %>%
  group_by(test, variance) %>%
  summarise(TypeI = mean(pvalue <= .05, na.rm = TRUE), meanStat = mean(stat, na.rm = TRUE), N = sum(!is.na(pvalue))) %>%
  arrange(desc(TypeI)) %>%
  mutate(
    TypeI = round(TypeI, 2),
...
ggplot(temp, aes(x = variance, y = TypeI)) +
  geom_point() +
  geom_hline(yintercept = .05) +
...
The skewness test shows a clear increase in type I error rates as the underlying distribution becomes more skew. This is a major drawback as this is the exact characteristic the test should be able to detect. The test does not achieve the target 5% error rate for any range of skew.

```{r empSkewnessTypeI, echo=FALSE}
typeI <- readRDS("results/empirical_skewness_type_one.rds")
...
ggplot(temp, aes(x = skew, y = TypeI)) +
  geom_point() +
  geom_hline(yintercept = .05) +
  scale_x_continuous(breaks = seq(-.9, .9, .2), limits = c(-.9, .9)) +
  scale_y_continuous(breaks = seq(0, 1, .01), limits = c(0, .10)) +
  labs(x = "Skew", y = "Type I Error")
...
```{r empKurtosisTypeI, echo=FALSE}
typeI <- readRDS("results/empirical_kurtosis_type_one.rds")

temp <- typeI %>%
  group_by(test, kurtosis) %>%
  summarise(TypeI = mean(pvalue <= .05, na.rm = TRUE), meanStat = mean(stat, na.rm = TRUE), N = sum(!is.na(pvalue))) %>%
  arrange(desc(TypeI)) %>%
  mutate(
    TypeI = round(TypeI, 2),
...
ggplot(temp, aes(x = kurtosis, y = TypeI)) +
  geom_point() +
  geom_hline(yintercept = .05) +
  scale_x_continuous(breaks = seq(-.9, .9, .2), limits = c(-.9, .9)) +
  scale_y_continuous(breaks = seq(0, 1, .01), limits = c(0, .30)) +
  labs(x = "Kurtosis", y = "Type I Error")
