...
```
# Data Overview
Similar to type I error rates, type II error rates are estimated. The main change is the alternative hypothesis is true. The null hypothesis is held constant and effect size varies. The first five rows look like
...
Like type I calculations, each simulated experiment is based on a sample size of 200. Each combination of effect size and test are repeated 2,000 times. Where possible, exact tests are included for comparison.
...
To check the above, one graph is shown per test. When the effect size is near 0, type II error rates are near  100%. As effect sizes grow, type II error rates decrease. All tests achieve near 0% type II for large effect sizes.
