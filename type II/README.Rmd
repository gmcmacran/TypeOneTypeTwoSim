...
Most tests can achieve a near 0% type II error for a large enough effect size. The test for a geometric distribution's s p and a gaussian's variance stand out as not getting close to zero. The gaussian variance test could hit zero if larger effect sizes were ran. Nearly the entire range of p was tested for the geometric test and it did not get close to zero.
...
temp %>%
  ggplot(aes(x = effectSize, y = TypeII)) +
  geom_point() + 
  scale_x_continuous(breaks = seq(-.5, .5, .10)) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
...
temp %>%
  ggplot(aes(x = effectSize, y = TypeII)) +
  geom_point() + 
  scale_x_continuous(breaks = seq(-.15, .15, .04)) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
...
## Gaussian
Type II error rate is surprisingly high for both the exact test and the likelihood ratio test.

```{r gaussainTypeII2, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeII <- bind_rows(
  readRDS("results/gaussian_type_two.rds"),
  readRDS("results/gaussian_type_two_exact.rds")
)
...
temp %>%
  ...
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
  labs(x = "Effect Size", y = "Type II Error", caption = "Null: variance = 5") + 
  facet_grid(rows = vars(alt), cols = vars(test))
...
## Geometric
For the greater than alternative, this test has 100% type II error when the null is .05. For all alternatives it never does better than .20%. This test unusable. 
