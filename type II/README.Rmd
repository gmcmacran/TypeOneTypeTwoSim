...
Like type I calculations, each simulated experiment is based on a sample size of 500. Each combination of effect size and test is repeated 5,000 times. Where possible, exact tests are included for comparison.
...
typeII <- typeII %>%
  mutate(test = factor(test, levels = c("gaussian_mu_test", "t.test", "gaussian_variance_test", "varTest")))
...
## Log Normal
```{r logNormalTypeII, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeII <- readRDS("results/log_normal_type_two.rds")

typeII <- typeII %>%
  mutate(test = factor(test, levels = c("log_normal_mu_test", "log_normal_variance_test")))
...
temp %>%
  ggplot(aes(x = effectSize, y = TypeII)) +
  geom_point() +
  scale_x_continuous(breaks = seq(-.9, .9, .20)) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
...
temp %>%
  ggplot(aes(x = effectSize, y = TypeII)) +
  geom_point() +
  scale_x_continuous(breaks = seq(-.12, .12, .04)) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
...
temp %>%
  ggplot(aes(x = effectSize, y = TypeII)) +
  geom_point() +
  scale_x_continuous(breaks = seq(-.20, .20, .05)) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
  labs(x = "Effect Size", y = "Type II Error", caption = "Null: p = .50") +
  facet_grid(rows = vars(alt), cols = vars(test))
...
```{r empVarianceTypeII, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeII <- readRDS("results/empirical_variance_type_two.rds")
...
temp %>%
  ggplot(aes(x = effectSize, y = TypeII)) +
  geom_point() +
  scale_x_continuous(breaks = seq(-1, 1, .20), labels = comma) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
  labs(x = "Effect Size", y = "Type II Error", caption = "Null: variance = 4") +
  facet_grid(rows = vars(alt), cols = vars(test))
...
```{r empQuantTypeII, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeII <- readRDS("results/empirical_quantile_type_two.rds")
...
temp %>%
  ...
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
  labs(x = "Effect Size", y = "Type II Error", caption = "Null: median = 3") +
  facet_grid(rows = vars(alt), cols = vars(test))
