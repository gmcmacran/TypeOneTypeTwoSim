...
typeII <- typeII %>%
  mutate(test = factor(test, levels = c("log_normal_mu_one_sample", "log_normal_variance_one_sample")))
...
```{r empSkewTypeII, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeII <- readRDS("results/empirical_skewness_type_two.rds")
...
temp %>%
  ...
  scale_x_continuous(breaks = seq(-1, 1, .20), labels = comma) +
  scale_y_continuous(breaks = seq(0, 1, .20), limit = c(0, 1)) +
  labs(x = "Effect Size", y = "Type II Error", caption = "Null: skewness = 0") +
  facet_grid(rows = vars(alt), cols = vars(test))
...
```{r empKurtosisTypeII, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
typeII <- readRDS("results/empirical_kurtosis_type_two.rds")
