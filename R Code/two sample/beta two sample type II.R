...
run_sim <- function(shape1EffectSizes) {
  sim_results <- tibble()
...
sim_results_part_one <- future_map_dfr(shape1EffectSizes, run_sim, .options = furrr_options(seed = TRUE))

shape2EffectSizes <- seq(-2, 2, .25) %>%
  setdiff(0)

run_sim <- function(shape2EffectSizes) {
  sim_results <- tibble()
...
sim_results_part_two <- future_map_dfr(shape2EffectSizes, run_sim, .options = furrr_options(seed = TRUE))
