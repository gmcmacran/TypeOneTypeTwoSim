...
# save
sim_results %>%
  saveRDS("results/empirical_type_one_one_way.rds")
...
sim_results %>%
  filter(test == "empirical_mu_one_way") %>%
  distinct(effectSize) %>%
...
# save
sim_results %>%
  saveRDS("results/empirical_type_two_one_way.rds")
