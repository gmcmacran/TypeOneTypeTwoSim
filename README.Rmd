...
Each simulated experiment is based on a sample size of 500. Each combination of true hypothesis, parameter value, and test is repeated 5,000 times.
...
The empirical tests for skewness and kurtosis are two exceptions. The simulation shows type I error is above the target .05.
...
Again, the empirical tests for skewness and kurtosis are exceptions. They are well above the target .05 rate.
...
empLikeMu <- readRDS("results/empirical_mu_type_one_one_way.rds")
empLikeMu <- empLikeMu %>%
  select(test, alt, param = mu, pvalue)

empLikeVariance <- readRDS("results/empirical_variance_type_one_one_way.rds")
empLikeVariance <- empLikeVariance %>%
  select(test, alt, param = variance, pvalue)

empLikeSkew <- readRDS("results/empirical_skewness_type_one_one_way.rds")
empLikeSkew <- empLikeSkew %>%
  select(test, alt, param = skew, pvalue)

empLikeKurtosis <- readRDS("results/empirical_kurtosis_type_one_one_way.rds")
empLikeKurtosis <- empLikeKurtosis %>%
  select(test, alt, param = kurtosis, pvalue)
...
log_normal <- readRDS("results/log_normal_type_two.rds")
...
beta <- bind_rows(
  readRDS("results/beta_type_two_shape1.rds"),
  readRDS("results/beta_type_two_shape2.rds")
)
...
empLikMu <- readRDS("results/empirical_mu_type_two.rds")

empLikVariance <- readRDS("results/empirical_variance_type_two.rds")

empLikSkew <- readRDS("results/empirical_skewness_type_two.rds")

empLikKurtosis<- readRDS("results/empirical_kurtosis_type_two.rds")

empLikQuant <- readRDS("results/empirical_quantile_type_two.rds")
...
log_normal <- readRDS("results/log_normal_type_two_one_way.rds")
...
empLikMu <- readRDS("results/empirical_mu_type_two_one_way.rds")

empLikVariance <- readRDS("results/empirical_variance_type_two_one_way.rds")

empLikSkew <- readRDS("results/empirical_skewness_type_two_one_way.rds")

empLikKurtosis<- readRDS("results/empirical_kurtosis_type_two_one_way.rds")

empLikQuant <- readRDS("results/empirical_quantile_type_two_one_way.rds")
...
The worst performing confidence intervals are the empirical skewness and kurtosis intervals which are meaningfully off from the target 95%. This is inline with the inflated type I error rates.
...
empLikMu <- empLikMu %>%
  mutate(
    CI_LB = if_else(is.na(CI_LB), -Inf, CI_LB),
...
empLikSkew <- empLikSkew %>%
  ...
  select(test, alt, param = skew, CI_LB, CI_UB)
...
empLikKurtosis <- empLikKurtosis %>%
  ...
  select(test, alt, param = kurtosis, CI_LB, CI_UB)
...
empLikQuant <- empLikQuant %>%
  ...
  select(test, alt, param = value, CI_LB, CI_UB)
...
ggplot(temp, aes(x = covRate, y = test2, color = factor(type))) +
  ...
  scale_x_continuous(breaks = seq(0, 1, .05), limits = c(.80, 1)) +
